...
import { ItemCatalogEntry, ItemType, ItemTypeShort } from '../types';
...
const Items: React.FC = () => {
  ...
  const [currentItem, setCurrentItem] = useState<ItemCatalogEntry>({
      id: '', name: '', type: ItemType.NORMAL, publicPrice: 0, pharmaPrice: 0
  });
  ...
  const handleSave = () => {
      ...
      loadItems();
      setIsEditing(false);
      setCurrentItem({ id: '', name: '', type: ItemType.NORMAL, publicPrice: 0, pharmaPrice: 0 });
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
            ...
            <button onClick={() => { setIsEditing(true); setCurrentItem({ id: '', name: '', type: ItemType.NORMAL, publicPrice: 0, pharmaPrice: 0 }) }} className="bg-primary-600 text-white px-4 py-2 rounded-xl hover:bg-primary-700 flex items-center gap-2 font-bold"><Plus size={18} /> صنف جديد</button>
        </div>
      </div>
          ...
          <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
              <table className="w-full text-right text-sm">
                  ...
                  <tbody className="divide-y">
                      ...
                      {filteredItems.length === 0 && <tr><td colSpan={5} className="p-8 text-center text-gray-400">لا توجد أصناف مطابقة</td></tr>}
                  </tbody>
              </table>
          </div>
      </div>
